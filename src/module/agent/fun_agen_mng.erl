...
-export([do_init/0,do_close/0,do_call/1,do_info/1,do_time/1,do_msg/1,start_fly/3,server_delete_items/3]).
...
do_msg({agent_in,Usr,Sid,Ip,InitData,AgentHid,AgentIndex}) ->
	...
	db:dirty_put(#ply{uid=Usr#usr.id, aid=Usr#usr.acc_id, sid=Sid, name=Usr#usr.name,
					  prof=Usr#usr.prof, lev=Usr#usr.lev, ip=Ip, agent_pid=AgentHid,agent_idx=AgentIndex,
					  fight_score=InitData#agent_init_data.fight_score}),
