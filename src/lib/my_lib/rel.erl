...
-export([createall/1, create/1, install/2]).
-define(BUFSIZE, 8192).
%% Note: RelFileName below is the *stem* without trailing .rel,
%% .script etc.

%% Verson = string.ex:"0.0.1"
createall(Version) when is_list(Version) ->
	Dbm = "dbm_v" ++ Version,
	Login = "login_v" ++ Version,
	MyNet = "mynet_v" ++ Version,
	Game = "gamemng_v" ++ Version,
	Public = "public_v" ++ Version,
	Report = "report_v" ++ Version,
	create(Dbm),
	io:format("create dbm ok.~n"),
	create(Login),
	io:format("create login ok.~n"),
	create(MyNet),
	io:format("create mynet ok.~n"),
	create(Game),
	io:format("create gamemng ok.~n"),
	create(Public),
	io:format("create public ok.~n"),
	create(Report),
	io:format("create report ok.~n"); 
createall(_Version) ->
	io:format("Error:param error,~p is not a version string,need a string like \"0.0.1\"~n", [_Version]).
